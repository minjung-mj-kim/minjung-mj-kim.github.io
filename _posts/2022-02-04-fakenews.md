...
[Go to my GitHub of this project](https://github.com/minjung-mj-kim/project/tree/master/fake_news)
...
-    BREAKING: IRAN Tests Cruise Missile…Trump WARNS…They’re “Playing with fire…They don’t appreciate how ‘kind’ President Obama was to them. Not Me!” [VIDEO]
-                                                      WHY DECISION LIBERAL JUDGE IN CONNECTICUT Is About To Make Could Be HUGE Threat To Our SECOND AMENDMENT
-                                                              VP Of Trump Transition Team To Dems: ‘Put Your Big Boy Pants On’ And Attend Inauguration (VIDEO)
-                                                                       Here’s A Look At The Obama Administration Scandal From This Point In His Administration
...
- Glove word embeddings, done to
    - "org_title": Raw text, which includes all noise
    - "lower_title": Lower cased raw text
    - "cleaned_words": Preprocessed text, before stopword removal and lemmatized
    - "minimal_words": Preprocseed text, after stopword removal and lemmatization

- One-hot encodings, done to
    - "cleaned_pos": part-of speech taggings of "cleaned_words" text
...
The final model showed good performance. The accuracy was 95%, shown as the blue line in the above plot. Both precision and recall showed similar scores. Each element model of the final ensemble showed good performance as well (each bar graph in the above figure). Each model showed accuracy over 80% while showing a high correlation in prediction between models. Variation in the text cleaning process or machine learning architecture didn't affect overall performance significantly.
